...
pub struct OnboardingPayload {
    ...
    #[serde(alias = "habitKind")]
    pub habit_kind: Option<String>,
}
...
pub(crate) fn complete_onboarding_record(
    conn: &Connection,
    ...
    let mut year_goal_id: Option<String> = None;
    ...
    if let (Some(title), Some(why_raw)) = (year_title, why_raw) {
        ...
        warning = w;
        year_goal_id = Some(year_id.clone());
        year_area_id = Some(area_id.to_string());

        let mut parent_id = year_id;
        if let Some(quarter_title) = optional_goal_title(payload.quarter_title.as_deref())? {
            let (id, w) =
                insert_goal(&tx, &quarter_title, &why, area_id, "quarter", Some(&parent_id), year)?;
            if warning.is_none() {
                warning = w;
            }
            parent_id = id;
        }
        if let Some(month_title) = optional_goal_title(payload.month_title.as_deref())? {
            let (id, w) =
                insert_goal(&tx, &month_title, &why, area_id, "month", Some(&parent_id), year)?;
            if warning.is_none() {
                warning = w;
            }
            parent_id = id;
        }
        if let Some(week_title) = optional_goal_title(payload.week_title.as_deref())? {
            let (id, w) =
                insert_goal(&tx, &week_title, &why, area_id, "week", Some(&parent_id), year)?;
            if warning.is_none() {
                warning = w;
            }
            parent_id = id;
        ...
        if let Some(task_title) = optional_text(payload.task_title.as_deref()) {
            ...
            insert_task(&tx, &title, &day, Some(&parent_id), Some(&day), true)?;
        }
    }
    ...
    if let Some(habit_title) = optional_text(payload.habit_title.as_deref()) {
        ...
        let freq = payload.habit_frequency.as_deref().unwrap_or("daily");
        let target = if freq == "weekly" { 3 } else { 7 };
        let (frequency_type, frequency_target) = domain::normalize_frequency(freq, target)
            .map_err(|message| AppError::new(VALIDATION_FAILED, message))?;
        let kind = domain::normalize_habit_kind(payload.habit_kind.as_deref())
            .map_err(|message| AppError::new(VALIDATION_FAILED, message))?;
        let habit_goal = match (year_goal_id.as_deref(), year_area_id.as_deref()) {
            (Some(gid), Some(goal_area)) if goal_area == area_id => Some(gid),
            _ => None,
        };
        insert_habit(
            &tx,
...
pub fn complete_onboarding(
    ...
    db::with_conn(&db, |conn| complete_onboarding_record(conn, payload))
}
